...
| desktop | 465 (rs, html, js, css) | no: Tauri shell, non-RC | none | RETIRE under SYS-03 once the owner accepts ADR [0006](decisions/0006-re-expand-to-agent-memory-platform.md) (status proposed); frozen, not deleted, until then | 70 tracked files, no CI job |
| tests/ui | 2025 (.ts) | no: browser UI e2e specs whose CI job was removed at d139313 | none | RETIRE under SYS-03 once the owner accepts ADR [0006](decisions/0006-re-expand-to-agent-memory-platform.md) (status proposed); frozen, not deleted, until then | d139313 |
...
| desktop (Tauri shell), web UI embed, tests/ui browser e2e specs, vdb-test-suite (69 tracked files) | 465 rs/html/js/css; 2025 ts | non-RC; the browser CI job was removed at d139313 | no | RETIRE under SYS-03 once the owner accepts ADR [0006](decisions/0006-re-expand-to-agent-memory-platform.md) (status proposed); frozen, not deleted, until then |
...
| SYS-01 | serverRuntime extracted from `cmd/deepdata/main.go`; NewVectorStore has zero callers | all canonical tests green with the legacy engine unconstructed |
