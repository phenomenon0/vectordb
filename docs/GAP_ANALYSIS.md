...
| D4 | `nine unary RPCs` | `README.md:23`, `internal/collection/API.md:210`, `docs/installation.md` :9 and :198, `docs/why-vectordb.md:11` | `api/proto/deepdata/v3/deepdata.proto` declared `11 unary RPCs` (rpc lines 241-251) at a99fe53, when Upsert and GetDoc landed | fixed by the 2026-09-01 docs rewrite; guarded by R1 |
| D5 | `five mutations`; `internal/collection/API.md:132` denies upsert on V3 | `internal/collection/API.md:11`, `docs/why-vectordb.md` :16 and :43, `README.md:21` (mutation row lacks upsert) | 6 journal mutations since a99fe53: CreateCollection, DeleteCollection, Insert, BatchInsert, DeleteDoc, Upsert ([2026-08-07-hardening-upsert](../tasks/journal/2026-08-07-hardening-upsert.md), ADR [0003](decisions/0003-upsert-and-get-doc-join-canonical.md)) | fixed by the 2026-09-01 docs rewrite; guarded by R2 and R4 |
...
| 12 | GET /v3/tenants/{t} returns 200 with zeros for an unknown tenant; correct but unsaid | a tenant exists iff it owns a collection; there is no create-tenant mutation among the 6 | CTL-04 (contract text) |
...
Durability classes (journal §5.2): **A canonical**: the 6 journal mutations, snapshot v2, schema; corruption fails closed. **B accreted signal**: usage frecency, later feedback weights; corruption is a loud discard (error log plus a status field) and the collection stays up. **C derived**: indexes; rebuilt from A on load.
