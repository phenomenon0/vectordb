...
Accepted reading (§5.10, :200; owner, 2026-09-01): "ubiquity" means reachable from any agent runtime over MCP/HTTP/gRPC/SDK, not a desktop app; no user of the desktop or browser UI exists yet, and the platform is planned for agents. On that reading desktop/, the web-UI embed (the go:embed of cmd/deepdata/web-ui/dist in cmd/deepdata/server.go, the browser UI's static files, not text embedding, which CTL-02 keeps live above the engine) and tests/ui are retired under SYS-03 behind the archive tag archive/pre-narrowing-v2.
