...
// TensorRefBridge maps Cowrie TensorRef <-> VectorDB.
...
func (b *TensorRefBridge) Store(ctx context.Context, doc string, meta map[string]string, id string) (*cowrie.Value, error) {
	if id == "" {
	...
	return cowrie.TensorRef(b.defaultStore, []byte(resp.ID)), nil
}

// Search returns TensorRefs sorted by similarity.
func (b *TensorRefBridge) Search(ctx context.Context, query string, topK int) ([]*cowrie.Value, []float32, error) {
	req := QueryRequest{
		Query:      query,
		TopK:       topK,
		Collection: b.collectionMap[b.defaultStore],
	...
	refs := make([]*cowrie.Value, len(resp.IDs))
	for i, id := range resp.IDs {
		refs[i] = cowrie.TensorRef(b.defaultStore, []byte(id))
	}
...
func (b *TensorRefBridge) Resolve(ctx context.Context, ref cowrie.TensorRefData) (string, map[string]string, error) {
	req := QueryRequest{
...
func (b *TensorRefBridge) BatchStore(ctx context.Context, docs []string, metas []map[string]string) ([]*cowrie.Value, error) {
...
func (b *TensorRefBridge) Delete(ctx context.Context, ref cowrie.TensorRefData) error {
