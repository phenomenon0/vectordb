...
Next action: RCV-05 — rerun the cgroup rehearsal (scripts/recovery_rehearsal.py) on a binary built from a tree that carries the empty-sparse-vector replay fix (the a683dc6 binary refused the preserved journal at LSN 480); Phase A first, then Phase B on the probe root once the host has 18 GiB available (10G cap plus the 8 GiB desktop reserve). Then SDK-02 and SOAK-01 (RCV-03, RCV-04 and RCV-06 gained tests and hardening cases on 2026-09-01; the platform slices CTL-01..05, SYS-01..04 and MED-01 all pass, MEM-02 waits for a harness).
...
- REP-01 — release gate: internal/replication ships in the RC and had no gate at all, and unlike the darwin port it is a feature on the gated target, so the release bar should include it; open until scripts/hardening_check.sh multinode has run and written a receipt.
