...
type Payload struct {
	...
	Docs           []string
	IDs            []string
	Seqs           []uint64 // Stable insertion sequence used by deterministic pagination
	...
	IndexTypes     map[string]string // Collection -> registered index factory type
	IndexDims      map[string]int    // Collection -> index vector dimension
	IndexChecksums map[string]string // Collection -> SHA-256 of serialized index blob
	Checksum       string
	LastSaved      time.Time
